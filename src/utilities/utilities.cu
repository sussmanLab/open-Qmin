...
/*!
Store the dot product of two dVecs in a double vec
*/
__global__ void gpu_vec_dot_product_kernel(dVec *input1, dVec *input2, double *output,int N)
    {
...
/*!
Store the dot product of two 5-component covectors in a double vector
*/
__global__ void gpu_vec_covectorDotProduct_kernel(dVec *input, double *output,int N)
    {
    unsigned int idx = blockDim.x * blockIdx.x + threadIdx.x;
    if (idx >= N)
        return;
...
/*!
Store the dot product of two 5-component vectors in a double vector
*/
__global__ void gpu_vec_vectorDotProduct_kernel(dVec *input1, dVec *input2, double *output,int N)
    {
    unsigned int idx = blockDim.x * blockIdx.x + threadIdx.x;
    if (idx >= N)
        return;
...
Store the dot product of two 5-component vectors in a double vector
*/
__global__ void gpu_vec_vectorDotProduct_kernel(dVec *input, double *output,int N)
    {
    unsigned int idx = blockDim.x * blockIdx.x + threadIdx.x;
    if (idx >= N)
        return;
...
/*!
Store the dot product of two dVecs in a double vec, unrolled by dimension
*/
__global__ void gpu_vec_dot_product_unrolled_kernel(dVec *input1, dVec *input2, double *output,int N)
    {
    unsigned int idx = blockDim.x * blockIdx.x + threadIdx.x;
...
__global__ void gpu_dVec_plusEqual_dVec_kernel(dVec *d_vec1,dVec *d_vec2,double factor,int n)
    {
    // read in the index that belongs to this thread
    unsigned int idx = blockDim.x * blockIdx.x + threadIdx.x;
    if (idx >= n)
...
double gpu_gpuarray_QT_covector_dot_product(
                        GPUArray<dVec> &input1,
                        GPUArray<double> &intermediate,
                        GPUArray<double> &intermediate2,
                        int N,
                        int block_size)
    {
    if (N == 0)
    ...
    double result = 0;
        ...
        ArrayHandle<double> inter1(intermediate,access_location::device,access_mode::overwrite);
        ...
        int maxBlocks = 64;
        int maxThreads = 256;
        ArrayHandle<double> inter2(intermediate2,access_location::device,access_mode::overwrite);
...
double gpu_gpuarray_QT_vector_dot_product(
                        GPUArray<dVec> &input1,
...
double gpu_gpuarray_dVec_dot_products(
                        GPUArray<dVec> &input1,
                        GPUArray<dVec> &input2,
                        GPUArray<double> &intermediate,
                        GPUArray<double> &intermediate2,
    ...
    if(true) // for testing...switch to best reduction kernel
        ...
        ArrayHandle<double> inter1(intermediate,access_location::device,access_mode::overwrite);
        ArrayHandle<double> inter2(intermediate2,access_location::device,access_mode::overwrite);
        ArrayHandle<double> answer(ans,access_location::device,access_mode::overwrite);
        gpu_dVec_dot_products(i1.data,i2.data,inter1.data,inter2.data,answer.data,0,N,block_size);
        }
        ArrayHandle<double> answer(ans,access_location::host,access_mode::read);
...
*/
bool gpu_dVec_dot_products(dVec *input1,dVec *input2, double *intermediate, double *intermediate2,double *output, int helperIdx, int N,int block_size)
    {
...
*/
bool gpu_dVec_dot_products(dVec *input1,dVec *input2, double *output, int helperIdx, int N)
    {
    //double init = 0.0;
    //dVecDotProduct mult_op;
    //thrust::plus<double> add_op;
    //thrust::device_ptr<double> ptrAns = thrust::device_pointer_cast(output);
...
template double gpuReduction<double>(int  n,int  numThreads,int  numBlocks,int  maxThreads,int  maxBlocks,double *d_idata,double *d_odata);
...
template bool gpu_copy_gpuarray<double>(GPUArray<double> &copyInto,GPUArray<double> &copyFrom,int maxBlockSize);
